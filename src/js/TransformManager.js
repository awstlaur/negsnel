...
TransformManager.prototype.displayBoxString = function () {
    return this.displayBox.getX().toString().
        concat(
            " ",
...
TransformManager.prototype.updateTransform = function () {
    const p = new Point(this.displayBox.getCenterX(), this.displayBox.getCenterY());
    ...
    const centerDest = new Point(transform.x(p.getX(), p.getY()), transform.y(p.getX(), p.getY()));
...
TransformManager.prototype.scale = function (scale, fix) {
    if (scale > 0) {
        const fixPoint = fix ||
            new Point(this.displayBox.getCenterX(),
                      this.displayBox.getCenterY());
        this.setDisplayBox(new Rectangle(
            (scale * (this.displayBox.getX() - fixPoint.getX())) + fixPoint.getX(),
            (scale * (this.displayBox.getY() - fixPoint.getY())) + fixPoint.getY(),
