...
import TilingEdge from "./tiling/TilingEdge";
...
export default class Orbit {
    private s: Point;
    private dir: Point;
    private edge: TilingEdge;
    constructor(startingPolygon: TilingPolygon, start: Point, end: Point) {
        this.s = start;
    ...
    public next(): Point {
        ...
        while (Geometry.triangleSign(this.s, this.edge.end(), e) >= 0) {
            ...
            if (this.edge.getI() === first) {
                throw new Error(
                    "Failed to continue path due to numerical errors!"
                );
            }
        }
